...
const ClassesPage = () => {
    ...
    try {
      ...
      const registrations =
        result?.data?.sectionRegistration?.edges?.map((el) => {
          ...
          return { ...el, grade: gradeObj?.grade || "" };
        }) || [];
      ...
      setSelectedTerm(termFromQuery || sortedTerms[0] || null);
    } catch (err) {
  ...
  const handleCourseClick = (section, course, title, number) => {
    ...
    const courseData = {
      ...
      subjectAbbreviation: course?.subject6?.abbreviation || "",
    ...
    history.push(
      `/coursepage?sectionId=${section?.id}&extension_canvas_url=${extension_canvas_url}`
    );
  };
  ...
  return (
    <div style={{ padding: "1rem" }}>
      ...
      >
        ...
        <TextLink
          id={`EllucianEnabled`}
          href={extension_canvas_url || "https://www.ellucian.com/"}
          target={"__blank"}
        >
          Canvas →
        </TextLink>
        ...
        {sectionsForTerm.map(({ node, grade }, index) => {
          ...
          return (
            ...
            >
              <div>
                <TextLink
                  id={`EllucianEnabled-${index}`}
                  onClick={() =>
                    handleCourseClick(section, course, title, number)
                  }
                >
                  {courseName}
                </TextLink>
