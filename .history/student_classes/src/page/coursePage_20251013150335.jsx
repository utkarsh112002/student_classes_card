...
import mapImg from "../assets/image";
...
const CoursePage = (props) => {
  ...
  useEffect(() => {
    ...
    const fetchInstructionalEvents = async (sectionId) => {
      try {
        ...
        const resource = `GetInstructionalEvents-ServerlessAPI?${queryParams.toString()}`;
        const response = await authenticatedEthosFetch(resource, { method: 'GET' });

        if (!response.ok) throw new Error(`Failed to fetch instructional events`);
        const data = await response.json();

        const events =
          Array.isArray(data) ? data :
          data.data || data.payload || data.instructionalEvents || data.message || [];
    ...
    const fetchInstructors = async (sectionId) => {
      try {
        const queryParams = new URLSearchParams();
        queryParams.append('sectionId', sectionId);
        queryParams.append('cardId', cardId);
        ...
        const instructorsArr = Array.isArray(data) ? data :
          data.data || data.payload || data.instructors || data.message || [];
        if (isMounted) setInstructors(instructorsArr);
      } catch (err) {
        console.error(err);
  ...
  if (loading) {
    return <Typography style={{ padding: spacing30 }}>Loading course details…</Typography>;
  }

  return (
    <div style={{ padding: spacing30, maxWidth: 900, margin: "0 auto" }}>
      {/* ────────── COURSE HEADER ────────── */}
      {courseData && (
        <Card>
          <CardHeader
            title={courseData.courseName}
            subtitle={`${courseData.subjectAbbreviation} - ${courseData.subjectTitle}`}
          />
          <CardContent>
            <Typography variant="body2">
              Start Date: {formatDate(courseData.startOn)} | End Date: {formatDate(courseData.endOn)}
            </Typography>
            {courseData.academicPeriod && (
              <Typography variant="body2" style={{ marginTop: "0.25rem" }}>
                Period: {formatDate(courseData.academicPeriod.startOn)} -{" "}
                {formatDate(courseData.academicPeriod.endOn)}
              </Typography>
            )}
          </CardContent>
      ...
      <div style={{ marginTop: spacing30 }}>
        <Typography variant="h4" style={{ marginBottom: "0.5rem" }}>Instructor</Typography>
        <Divider />
        {instructors.length === 0 ? (
          <Typography>No instructor information available.</Typography>
        ) : (
          instructors.map((instructor, idx) => {
            const inst = instructor.node?.instructor12 || instructor;
            const name = getPreferredName(inst?.names);
            const email = getPreferredEmail(inst?.emails);
            const phone = getPreferredPhone(inst?.phones);

            return (
              <Card key={idx} style={{ marginTop: "1rem" }}>
                <CardContent>
                  <Typography variant="h5" style={{ marginBottom: "0.5rem" }}>{name}</Typography>
      ...
      {/* ────────── SCHEDULE ────────── */}
      <div style={{ marginTop: spacing30 }}>
        <Typography variant="h4" style={{ marginBottom: "0.5rem" }}>Class Schedule</Typography>
        <Divider />
        {instructionalEvents.length === 0 ? (
          ...
          instructionalEvents.map((event, idx) => (
            <Card key={idx} style={{ marginTop: "1rem" }}>
              <CardContent>
                {event.instructionalMethod?.title && (
                  <Typography variant="h5">{event.instructionalMethod.title}</Typography>
                )}
                {event.recurrence?.timePeriod && (
                  <Typography>
                    {event.recurrence.timePeriod.startOn} – {event.recurrence.timePeriod.endOn}
                ...
                {event.recurrence?.repeatRule?.daysOfWeek && (
                  <Typography>
                    Days: {event.recurrence.repeatRule.daysOfWeek.join(", ")}
                  </Typography>
                )}
                {event.locations && event.locations.length > 0 && (
                  <Typography>
                    ...
                    {event.locations.map((loc, i) => (
                      <span key={i}>
                        {loc.location?.building?.title} {loc.location?.room?.title}
                      </span>
                    ))}
                  </Typography>
                )}
      ...
      {/* ────────── CAMPUS MAP (STATIC IMAGE) ────────── */}
      <div style={{ marginTop: spacing30 }}>
        <img
          src="https://www.gulfcoast.edu/images/Map_Panama_City_Campus.jpg"
          alt="Campus Map"
      ...
      {/* ────────── BACK BUTTON ────────── */}
      <div style={{ marginTop: spacing30, textAlign: "right" }}>
