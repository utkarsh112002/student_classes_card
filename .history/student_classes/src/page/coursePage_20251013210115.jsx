...
const CoursePage = (props) => {
  ...
  useEffect(() => {
    ...
    if (storedData) {
      try {
        ...
        if (parsedData.section?.id && sectionId && parsedData.section.id !== sectionId) {
          console.warn("⚠️ Section ID mismatch:", { fromURL: sectionId, fromCourseData: parsedData.section.id });
        }
      } catch (err) {
        console.error("Error parsing stored course data:", err);
        setPageTitle("Course Details");
  ...
  useEffect(() => {
    ...
    const fetchInstructionalEvents = async (sectionId) => {
      ...
      try {
        ...
        const resource = `GetInstructionalEvents-ServerlessAPI?${queryParams.toString()}`;
        const response = await authenticatedEthosFetch(resource, { method: 'GET', headers: { 'Accept': 'application/json', 'Content-Type': 'application/json' } });
        const data = await response.json();
        ...
        if (Array.isArray(data) && data.length > 0) {
          events = data;
        } else if (Array.isArray(data.instructionalEvents) && data.instructionalEvents.length > 0) {
          events = data.instructionalEvents;
    ...
    const fetchInstructorsFromServerless = async (sectionId) => {
      ...
      try {
        ...
        if (Array.isArray(data)) {
          ...
          instructorData = data.instructionalEvents;
        ...
        const instructorsWithDetails = await Promise.all(
          mappedInstructors.map(async (instructor) => {
            try {
              ...
              const personResource = `GetPersonDetails-ServerlessAPI?${personQueryParams.toString()}`;
              const personResponse = await authenticatedEthosFetch(personResource, { method: 'GET', headers: { 'Accept': 'application/json', 'Content-Type': 'application/json' } });
              const personData = await personResponse.json();

              let person = null;
              if (personData.id) person = personData;
              else if (personData.data?.id) person = personData.data;
              else if (personData.payload?.id) person = personData.payload;
              else if (Array.isArray(personData) && personData.length > 0) person = personData[0];
              else if (Array.isArray(personData.data) && personData.data.length > 0) person = personData.data[0];
              else if (Array.isArray(personData.payload) && personData.payload.length > 0) person = personData.payload[0];

              if (person?.id) {
                return { ...instructor, node: { ...instructor.node, instructor12: { id: personId, credentials: person.credentials || [], names: person.names || [], emails: person.emails || [], phones: person.phones || [], addresses: person.addresses || [] } } };
              }
  ...
  if (loading) return <div style={{ padding: "1.5rem", fontFamily: "Arial, sans-serif" }}>Loading course details...</div>;
  ...
  return (
    <div style={{ padding: "1.5rem", fontFamily: "Arial, sans-serif", backgroundColor: "#f8f9fa", minHeight: "100vh" }}>
      <div style={{ display: "flex", gap: "2rem", maxWidth: "1400px", margin: "0 auto" }}>
        ...
        <div style={{ flex: "0 0 40%", backgroundColor: "white", padding: "2rem", borderRadius: "8px", boxShadow: "0 2px 8px rgba(0,0,0,0.1)" }}>
          {courseData && (
            <div style={{ marginBottom: "2rem" }}>
              ...
              <ScheduleLocationInfo instructionalEvents={instructionalEvents} courseData={courseData} />
            </div>
          )}
          ...
          {/* Back Button */}
          <div style={{ marginTop: "2rem" }}>
            <button onClick={() => history.goBack()} style={{ padding: "0.6rem 1.5rem", backgroundColor: "#0066cc", color: "white", border: "none", borderRadius: "4px", cursor: "pointer", fontSize: "0.95rem", fontWeight: "500" }}
        ...
        <div style={{ flex: "0 0 60%", height: "400px", display: "flex", justifyContent: "center", alignItems: "center", background: "#fff", borderRadius: "8px", boxShadow: "0 2px 8px rgba(0,0,0,0.1)", overflow: "hidden" }}>
          <img src={campusImage} alt="Campus Map" style={{ width: "100%", height: "100%", objectFit: "fill" }} />
        </div>
      </div>
    </div>
  );
