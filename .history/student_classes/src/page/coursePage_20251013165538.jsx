...
// import { usePageControl, useData } from "@ellucian/experience-extension-utils";
// import { useLocation, useHistory } from "react-router-dom";
// import campusImage from "../assets/image.png"; // ✅ import your image here
...
//         queryParams.append(
//           "ethosAPIKey",
//           "2e5330bd-483a-42c8-925b-d59edf93345f"
//         );
...
//         else if (data.payload) events = data.payload;
//         else if (data.instructionalEvents) events = data.instructionalEvents;
//         else if (data.message) events = data.message;
...
//         if (!response.ok) {
//           throw new Error(`Failed to fetch instructors: HTTP ${response.status}`);
//         }

//         const data = await response.json();
//         let instructorData = [];
//         if (Array.isArray(data)) instructorData = data;
//         else if (data.data) instructorData = data.data;
//         else if (data.payload) instructorData = data.payload;
//         else if (data.message) instructorData = data.message;
//         else if (data.instructors) instructorData = data.instructors;

//         if (instructorData.length === 0) {
//           if (isMounted) {
//             setInstructors([]);
//             setError("No instructors found for this section.");
//           }
//           return;
//         }

//         // For simplicity we’ll just set the instructor data directly
//         if (isMounted) {
//           setInstructors(
...
//                         {event.recurrence.timePeriod.endOn}
//                       </div>
//                     )}
//                     {event.recurrence.repeatRule?.daysOfWeek && (
//                       <div style={{ marginTop: "0.25rem" }}>
...
//       const building = loc?.building?.title || "";
//       const room = loc?.room?.title || "";
      
//       if (building && room) {
//         return `Panama City Campus, ${building} ${room}`;
//       } else if (building) {
//         return `Panama City Campus, ${building}`;
//       }
//     }
//     return "Panama City Campus";
//   };
...
//         <div style={{ flex: "0 0 65%", backgroundColor: "white", padding: "2rem", borderRadius: "8px", boxShadow: "0 2px 8px rgba(0,0,0,0.1)" }}>
...
//                 marginBottom: "0.5rem"
//               }}>
//                 {formatSchedule(instructionalEvents)}
//               </div>
...
//                 marginBottom: "1.5rem"
//               }}>
//                 {getCampusLocation(instructionalEvents)}
//               </div>
//             </div>
//           )}
...
//                 <div><strong>Email:</strong> [instructor's employee email]</div>
//                 <div><strong>Office Location:</strong> [building] [room number]</div>
//                 <div><strong>Office Phone:</strong> [phone number] [extension]</div>
//               </div>
//             )}
//           </div>
...
const CoursePage = (props) => {
  ...
  // Load stored course data for page title
  useEffect(() => {
    const storedData = localStorage.getItem("selectedCourse");
    if (storedData) {
      try {
  ...
  // Fetch section, instructor, and event data
  useEffect(() => {
    let isMounted = true;
    ...
    const fetchInstructorsFromServerless = async (sectionId) => {
      try {
        ...
        const instructorsWithDetails = await Promise.all(
          mappedInstructors.map(async (instructor) => {
            try {
              ...
              if (!personResponse.ok) return instructor;

              const personData = await personResponse.json();
              let person = null;

              if (personData.id) person = personData;
              else if (personData.data && personData.data.id) person = personData.data;
              else if (personData.payload && personData.payload.id) person = personData.payload;
              else if (personData.message && personData.message.id) person = personData.message;
              else if (Array.isArray(personData) && personData.length > 0) person = personData[0];
        ...
        if (isMounted) setInstructors(instructorsWithDetails);
      } catch (err) {
        console.error(err);
        if (isMounted) {
          setInstructors([]);
  ...
  const getOfficeLocation = (addresses) => {
    if (!addresses || addresses.length === 0) return null;
    const officeAddr = addresses.find(
      (a) => a.type?.category === "business" || a.type?.category === "office"
    );
    const addr = officeAddr || addresses[0];
    if (addr) {
      const building = addr.addressLines?.[0] || "";
  ...
  const getCampusLocation = (events) => {
    ...
    if (event.locations && event.locations.length > 0) {
      ...
      const building = loc?.building?.title || "";
      const room = loc?.room?.title || "";
      if (building && room) return `Panama City Campus, ${building} ${room}`;
      else if (building) return `Panama City Campus, ${building}`;
    }
    return "Panama City Campus";
  };
  ...
  return (
    <div style={{ padding: "1.5rem", fontFamily: "Arial, sans-serif", backgroundColor: "#f8f9fa", minHeight: "100vh" }}>
      <div style={{ display: "flex", gap: "2rem", maxWidth: "1400px", margin: "0 auto" }}>
        {/* LEFT COLUMN - Course Info */}
        <div style={{ flex: "0 0 65%", backgroundColor: "white", padding: "2rem", borderRadius: "8px", boxShadow: "0 2px 8px rgba(0,0,0,0.1)" }}>
          {courseData && (
            <div style={{ marginBottom: "2rem" }}>
              <h1 style={{ fontSize: "1.5rem", fontWeight: "bold", margin: "0 0 0.5rem 0", color: "#333" }}>
                {courseData.subjectAbbreviation || "AMH"} {courseData.courseNumber || "2010"} {courseData.courseName || "United States History I"}
              </h1>
              <div style={{ fontSize: "1rem", color: "#555", marginBottom: "0.5rem" }}>{formatSchedule(instructionalEvents)}</div>
              <div style={{ fontSize: "1rem", color: "#555", marginBottom: "1.5rem" }}>{getCampusLocation(instructionalEvents)}</div>
            </div>
          )}
          ...
          {/* Error Display */}
          {error && (
            <div style={{ marginTop: "1.5rem", padding: "1rem", backgroundColor: "#fff3cd", border: "1px solid #ffc107", borderRadius: "4px", color: "#856404", fontSize: "0.9rem" }}>
              <strong>Note:</strong> {error}
            </div>
          )}
          ...
          <div style={{ marginTop: "2rem" }}>
            <button
              onClick={() => history.goBack()}
              style={{ padding: "0.6rem 1.5rem", backgroundColor: "#0066cc", color: "white", border: "none", borderRadius: "4px", cursor: "pointer", fontSize: "0.95rem", fontWeight: "500" }}
              onMouseOver={(e) => (e.target.style.backgroundColor = "#0052a3")}
              onMouseOut={(e) => (e.target.style.backgroundColor = "#0066cc")}
            >
              ← Back to Classes
        ...
        {/* RIGHT COLUMN - Campus Map */}
        <div style={{ flex: "0 0 32%" }}>
          <img src={campusImage} alt="Campus Map" style={{ width: "100%", height: "auto", borderRadius: "8px", boxShadow: "0 2px 8px rgba(0,0,0,0.1)", objectFit: "contain" }} />
        </div>
      </div>
    </div>
  );
