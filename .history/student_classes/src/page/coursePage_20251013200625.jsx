...
const ScheduleLocationInfo = ({ instructionalEvents, courseData }) => {
  const formatSchedule = (events) => {
    ...
    if (event.recurrence?.timePeriod) {
      ...
      timeStr = start && end ? `${start} - ${end}` : null;
  ...
  return (
    <div>
      <div style={{ fontSize: "1rem", color: "#555", marginBottom: "0.5rem" }}>
        {hasDynamicSchedule
          ? `${scheduleData.days} ${scheduleData.time}`
          : staticSchedule}
      </div>
...
const CoursePage = (props) => {
  ...
  const getOfficeLocation = (addresses) => {
    ...
    const officeAddr = addresses.find(
      (a) =>
        a.type?.category === "business" || a.type?.category === "office"
    );
        ...
        >
          {courseData && (
            <div style={{ marginBottom: "2rem" }}>
              ...
              >
                {courseData.subjectAbbreviation || "AMH"}{" "}
                {courseData.courseNumber || "2010"}{" "}
                {courseData.courseName || "United States History I"}
              </h1>
          ...
          {courseData && (
            <div style={{ fontSize: "0.95rem", lineHeight: "1.8", color: "#333" }}>
              <div>
                ...
                {formatDate(courseData.startOn || courseData.academicPeriod?.startOn)}
              </div>
              <div>
                <strong>End Date:</strong>{" "}
                {formatDate(courseData.endOn || courseData.academicPeriod?.endOn)}
              </div>
            </div>
          )}
